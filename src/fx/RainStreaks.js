...
void main() {
  ...
  vec4 b = viewMatrix * vec4(top + uDir * len, 1.0);
  ...
  mvPosition.xy += side * position.x * max(uHalfWidth, uPixel * depth * 0.7);
...
void main() {
  ...
  #endif
  if (a < 0.003) discard;
  vec4 c = linearToOutputTexel(vec4(uColor, 1.0));
...
export class RainStreaks {
  ...
  setAmount(t) {
    ...
    this.geometry.instanceCount = RainStreaks.countFor(this.amount, this.opts.maxStreaks);
