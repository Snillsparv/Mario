...
// Piecewise-linear colour ramp over stops [f, r, g, b, ...] (f ascending), into out.
function ramp(stops, f, out) {
  ...
  for (let s = 1; s < n; s++) {
    ...
    if (f <= f1 || s === n - 1) {
      ...
      out.r = stops[s * 4 - 3] + (stops[s * 4 + 1] - stops[s * 4 - 3]) * t;
      out.g = stops[s * 4 - 2] + (stops[s * 4 + 2] - stops[s * 4 - 2]) * t;
      out.b = stops[s * 4 - 1] + (stops[s * 4 + 3] - stops[s * 4 - 1]) * t;
      return;
    }
  }
}

const FLAME_RAMP = [0, 1.7, 1.25, 0.5, 0.3, 1.45, 0.62, 0.14, 0.7, 0.95, 0.2, 0.04, 1, 0.45, 0.06, 0.02];
const FIREBALL_RAMP = [0, 3.2, 2.5, 1.3, 0.2, 2.3, 1.2, 0.3, 0.55, 1.25, 0.33, 0.06, 1, 0.25, 0.06, 0.03];
const SPARK_RAMP = [0, 2.8, 2.3, 1.4, 0.5, 2.0, 0.9, 0.25, 1, 1.2, 0.3, 0.05];
const SMOKE_RAMP = [0, 0.085, 0.075, 0.07, 1, 0.19, 0.19, 0.2];
...
export function particleStyle(pool, i, out) {
  ...
  switch (kind) {
    case KIND.FLAME: {
      ...
      ramp(FLAME_RAMP, f, out);
      a = smooth(f / 0.1) * (1 - smooth((f - 0.55) / 0.45)) * heat;
      // Grows quickly, then licks upward thinner; flickers on its own phase.
      size = (s0 + (s1 - s0) * f) * (0.75 + 0.25 * smooth(f / 0.2)) * (0.86 + 0.14 * Math.sin(clock * 23 + seed * 60));
      break;
    }
    case KIND.SMOKE: {
      ramp(SMOKE_RAMP, f, out);
      // Young smoke is lit from below by the fire.
      const lit = Math.max(0, 1 - f * 4);
      out.r += 0.4 * lit;
      out.g += 0.13 * lit;
      out.b += 0.03 * lit;
      a = smooth(f / 0.15) * (1 - f) ** 1.2 * 0.78 * heat;
      size = s0 + (s1 - s0) * (1 - (1 - f) * (1 - f));
      break;
    }
    case KIND.EMBER: {
      out.r = 2.4;
    ...
    case KIND.SPARK: {
      ramp(SPARK_RAMP, f, out);
      a = (1 - f) ** 0.7 * heat;
      break;
    }
    case KIND.FIREBALL: {
      if (seed > 0.5) out.shape = SHAPE.FLAME_B;
      ramp(FIREBALL_RAMP, f, out);
      a = (1 - f * f) * heat;
      size = s0 + (s1 - s0) * (1 - (1 - f) ** 3);
      break;
    }
    case KIND.FLASH: {
      out.r = 3.2 - 1.4 * f;
      ...
      out.b = 1.2 - 1.0 * f;
      a = (1 - f) ** 1.6 * heat;
      size = s0 + (s1 - s0) * (1 - (1 - f) ** 4);
      break;
    }
    case KIND.DEBRIS: {
      // Charred chunks, still glowing a little at first.
      const glow = Math.max(0, 1 - f * 3);
      out.r = 0.07 + 0.5 * glow;
    ...
    case KIND.BOLT_GLOW: {
      ...
      out.g = 0.65;
      out.b = 1.2;
      a = boltFlicker(age, life) * heat * 0.7;
      break;
    }
    default:
      return false;
