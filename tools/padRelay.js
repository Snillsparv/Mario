...
// on every non-internal IPv4 address of this machine (likely home/office LAN addresses first,
// VPN / container / link-local ones last) at the port the server listens on, then the URL
// from the request's Host header as a fallback. The game appends ?room=CODE for the QR code.
...
// interfaces: os.networkInterfaces() shape. Returns the pad page URL for every usable IPv4
// address (best guesses first), then `${scheme}://${host}/pad.html` if not already listed.
export function padPageUrls({ interfaces = {}, port, scheme = 'http', host } = {}) {
  const found = [];
  for (const [name, addrs] of Object.entries(interfaces || {})) {
    for (const a of addrs || []) {
      if (!a || a.internal || !(a.family === 'IPv4' || a.family === 4)) continue;
      if (!/^\d{1,3}(\.\d{1,3}){3}$/.test(a.address) || a.address.startsWith('127.')) continue;
...
// and returns { middleware(req, res, next) (answers PAD_INFO_PATH), close(), stats() }.
// options: RELAY_LIMITS overrides, `port` (the pad page port; default: the port the server
// listens on) and `networkInterfaces` (default os.networkInterfaces, replaceable in tests).
export function attachPadRelay(httpServer, options = {}) {
  ...
  function middleware(req, res, next) {
    ...
    const port =
      opts.port ?? (addr && typeof addr === 'object' ? addr.port : req.socket.localPort);
    let interfaces = {};
    try {
